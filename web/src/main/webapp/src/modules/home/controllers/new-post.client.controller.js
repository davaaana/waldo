...
mainApp.controller('NewPostController', ['$rootScope', '$scope', '$http', '$mdDialog', '$mdToast', 'UserService', 'PostService', '$location',
    function ($rootScope, $scope, $http, dialog, $mdToast, UserService, PostService, $location) {
        ...
        $scope.adsType = function (type) {
            setTimeout(function () {
                if (type != 'undefined') {
                    document.getElementsByClassName('add-ads')[0].style.backgroundColor = "#b9b9b9";
                    document.getElementsByClassName('md-container md-mode-determinate')[0].style.backgroundColor = "#E0DEDE";
                    document.getElementsByClassName('md-bar md-bar2')[0].style.backgroundColor = "#b9b9b9";
                }
                if (type == 'SENDER') {
                    document.getElementsByClassName('add-ads')[0].style.backgroundColor= "rgb(68,138,255";
                    ...
                    document.getElementsByClassName('new_post_active')[0].style.color = "rgb(68,138,255)";
                    document.getElementsByClassName('new_post_active')[0].getElementsByTagName("span")[0].style.backgroundColor = "rgb(68,138,255)";
                ...
                if (type == 'CARRIER') {
                    ...
                    document.getElementsByClassName('new_post_active')[0].style.color = "orange";
                    document.getElementsByClassName('new_post_active')[0].getElementsByTagName("span")[0].style.backgroundColor = "orange";
                }
            }, 100)
        }
        ...
        $scope.$watch('step', function (el) {
            switch (el) {
                case 1 :
                    $scope.newPostProgress = 33;
                    break;
                case 2 :
                    $scope.newPostProgress = 66;
                    break;
                case 3 :
                    $scope.newPostProgress = 66;
                    break;
                case 4 :
                    $scope.newPostProgress = 100;
                    break;
                case 5 :
                    $scope.newPostProgress = 100;
                    break;
        ...
        $scope.completePost = function (el) {
            ...
            if (!$scope.newPostDataCheck.policyId) {
                $('[name="policy"]').addClass('md-select-danger');
                $('#policy').removeClass('element-hide');
                $('#policy').addClass('element-show');
                return;
            }else{
                $('[name="policy"]').removeClass('md-select-danger');
                $('#policy').removeClass('element-show');
                $('#policy').addClass('element-hide');
            }
            ...
            $http.post(SERVICE_URL + '/post', data).success(function (data) {
                if (data.success == true) {
                    ...
                    window.location.href = "#/index/own-post"
                }
                else {
